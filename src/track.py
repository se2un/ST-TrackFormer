...
def main(seed, dataset_name, obj_detect_checkpoint_file, tracker_cfg,
    ...
    if obj_detector_model is None:
        ...
        # Always enable both attention modules (Case 1: proposed model).
        obj_detect_args.use_spatial_attention = True
        obj_detect_args.use_temporal_attention = True
        ...
        for k, v in obj_detect_state_dict.items():
            ...
            if 'track_encoding' not in k:
                if 'transformer.encoder.layers' in k:
                    parts = k.split('.')
                    if len(parts) > 4 and parts[4] in ['spatial_attn', 'temporal_attn', 'norm_spatial', 'norm_temporal']:
                        new_state_dict[k] = v
                else:
                    new_state_dict[k] = v

        obj_detector.load_state_dict(new_state_dict, strict=False)
