...
def generate_coco_from_mot(split_name='train', seqs_names=None,
                           ...
                           frame_range=None, data_root='data/MOT16'):
    """
    ...
    for seq in seqs:
        ...
        if mots:
            gt_file_path = os.path.join(
                ...
                seq.replace('MOT17', 'MOTS20', 'MOT16'),
                'gt',
                'gt.txt')
...
if __name__ == '__main__':
    ...
    if args.mots20:
        ...
        # Train/Val splits
        num_splits = 3
        val_ratio = 0.2  # 20% of sequences for validation
        ...
        for i in range(num_splits):
            ...
            for seq in train_seqs:
                seq_length = len(os.listdir(os.path.join(data_root, 'train', seq, 'img1')))
                val_length = int(seq_length * val_ratio)
                val_seqs.append((seq, val_length))
                train_seqs_split.append((seq, seq_length - val_length))
    
            generate_coco_from_mot(
                f'mot16_train_split_{i+1}_coco', 
                seqs_names=[seq for seq, _ in train_seqs_split],
                data_root=data_root,
                root_split='train',
                frame_range={"start": 0, "end": 1 - val_ratio})
    
            generate_coco_from_mot(
                f'mot16_val_split_{i+1}_coco',
                seqs_names=[seq for seq, _ in val_seqs],
                data_root=data_root,
                root_split='train',
                frame_range={"start": 1 - val_ratio, "end": 1})
