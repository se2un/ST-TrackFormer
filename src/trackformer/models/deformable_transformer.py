...
class SpatialAttention(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.pos_encoding = nn.Parameter(torch.zeros(1, d_model, 1, 1))
    ...
    def forward(self, x, mask=None):
        ...
        pos_encoding = self.pos_encoding.expand(batch_size, -1, seq_length, -1).permute(0, 2, 1, 3)
        Q = Q + pos_encoding.squeeze(-1)
        K = K + pos_encoding.squeeze(-1)

        d_k = Q.size()[-1]
        scores = torch.matmul(Q, K.transpose(-2, -1)) / d_k ** 0.5

        if mask is not None:
            mask = mask.unsqueeze(1).expand(-1, seq_length, -1)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.1, activation="relu", n_levels=4, n_heads=8, n_points=4, use_spatial_attention=True):
        super().__init__()
        ...
        if self.use_spatial_attention:
            self.spatial_attn = SpatialAttention(d_model, n_heads)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, padding_mask=None):
        self_attn_output = self.self_attn(self.with_pos_embed(src, pos), reference_points, src, spatial_shapes, padding_mask)
        src = src + self.dropout1(self_attn_output)
        src = self.norm1(src)

        if self.use_spatial_attention:
            spatial_attn_output = self.spatial_attn(src, mask=padding_mask)
        else:
            spatial_attn_output = 0

        temporal_attn_output = self.temporal_attn(src.transpose(0, 1)).transpose(0, 1)
        attn_output = spatial_attn_output + temporal_attn_output
        src = src + self.dropout_spatial(attn_output)
        src = self.norm_spatial(src)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, src_padding_mask=None, query_attn_mask=None):
        ...
        self_attn_output = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1), key_padding_mask=query_attn_mask)[0].transpose(0, 1)
        tgt = tgt + self.dropout2(self_attn_output)
        tgt = self.norm2(tgt)

        temporal_attn_output = self.temporal_attn(tgt.transpose(0, 1)).transpose(0, 1)
        tgt = tgt + self.dropout_temporal(temporal_attn_output)
        tgt = self.norm_temporal(tgt)

        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos), reference_points, src, src_spatial_shapes, src_padding_mask)
...
def build_deforamble_transformer(args):
    ...
    return DeformableTransformer(
        ...
        # Spatial and Temporal Attention are always enabled (Case 1: Enc-SA + Enc-TA + Dec-TA).
