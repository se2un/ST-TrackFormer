...
class MOT20Sequence(Dataset):
    ...
    def _sequence(self) -> List[dict]:
        ...
        if osp.exists(det_file):
            ...
            # If detections file is not provided, use an empty detection for each frame
            dets = {i: np.empty((0, 5), dtype=np.float32) for i in range(1, self.seq_length + 1)}
    ...
    def write_results(self, results: dict, output_dir: str) -> None:
        """Write the tracks in the format for MOT20 sumbission
