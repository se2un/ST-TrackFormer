...
class MOT16Wrapper(Dataset):
    """A Wrapper for the MOT16_Sequence class to return multiple sequences."""

    def __init__(self, split: str, **kwargs) -> None:
        """Initializes all subsets of the dataset.

        Keyword arguments:
        split -- the split of the dataset to use (e.g., 'TRAIN', 'TEST', 'ALL')
        kwargs -- additional keyword arguments for the MOT16Sequence dataset
        """
        train_sequences = ['MOT16-02', 'MOT16-04', 'MOT16-05', 'MOT16-09', 'MOT16-10', 'MOT16-11', 'MOT16-13']
        test_sequences = ['MOT16-01', 'MOT16-03', 'MOT16-06', 'MOT16-07', 'MOT16-08', 'MOT16-12', 'MOT16-14']
        ...
        if split == "TRAIN":
            ...
            sequences = [f"MOT16-{split}"] if f"MOT16-{split}" in train_sequences + test_sequences else None

        if sequences is None:
            raise NotImplementedError(f"MOT16 split not available: {split}")

        self._data = []
        for seq in sequences:
            self._data.append(MOT16Sequence(seq_name=seq, **kwargs))
